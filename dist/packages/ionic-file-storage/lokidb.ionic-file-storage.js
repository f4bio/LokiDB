(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("@lokidb/ionic-file-storage", [], factory);
	else if(typeof exports === 'object')
		exports["@lokidb/ionic-file-storage"] = factory();
	else
		{ root["@lokidb/ionic-file-storage"] = factory(); root["LokiIonicFileStorage"] = root["@lokidb/ionic-file-storage"].default; }
...
class AvlTreeIndex {
    ...
    */
    ...
    collateRequest(node, range) {
        ...
        let cmp1 = this.comparator.compare(node.value, range.val);
        let cmp2 = 0;
        if (range.op === "$between") {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return RangedIndexFactoryMap; });
/* unused harmony export CreateJavascriptComparator */
/* unused harmony export CreateLokiComparator */
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return aeqHelper; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "e", function() { return ltHelper; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return gtHelper; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "f", function() { return sortHelper; });
/**
...
// EXTERNAL MODULE: ./packages/loki/src/helper.ts + 1 modules
...
const LokiOps = {
    ...
    $dteq(a, b) {
        return Object(helper["c" /* aeqHelper */])(a, b);
    },
    $gt(a, b) {
        return Object(helper["d" /* gtHelper */])(a, b, false);
    },
    $gte(a, b) {
        return Object(helper["d" /* gtHelper */])(a, b, true);
    },
    $lt(a, b) {
        return Object(helper["e" /* ltHelper */])(a, b, false);
    },
    $lte(a, b) {
        return Object(helper["e" /* ltHelper */])(a, b, true);
    },
    $between(a, range) {
        if (a === undefined || a === null)
            return false;
        return (Object(helper["d" /* gtHelper */])(a, range[0], true) && Object(helper["e" /* ltHelper */])(a, range[1], true));
    },
...
class result_set_ResultSet {
    ...
    simplesort(propname, options = { desc: false }) {
        ...
        const wrappedComparer = (a, b) => {
            return Object(helper["f" /* sortHelper */])(data[a][propname], data[b][propname], options.desc);
        };
        this._filteredRows.sort(wrappedComparer);
        return this;
    }
    ...
    _compoundeval(properties, obj1, obj2) {
        for (let i = 0, len = properties.length; i < len; i++) {
            const prop = properties[i];
            const field = prop[0];
            const res = Object(helper["f" /* sortHelper */])(obj1[field], obj2[field], prop[1]);
            if (res !== 0) {
                return res;
            }
        }
...
class collection_Collection extends event_emitter["a" /* LokiEventEmitter */] {
    ...
    static fromJSONObject(obj, options) {
        ...
        for (let ri in obj.rangedIndexes) {
            ...
            let rif = helper["b" /* RangedIndexFactoryMap */][sri.indexTypeName];
            // lookup comparator function in map based on comparator name
            let ricmp = helper["a" /* ComparatorMap */][sri.comparatorName];
    ...
    ensureRangedIndex(field, indexTypeName, comparatorName) {
        ...
        if (!helper["b" /* RangedIndexFactoryMap */][indexTypeName]) {
            throw new Error("ensureRangedIndex: Unknown range index type");
        }
        if (!helper["a" /* ComparatorMap */][comparatorName]) {
            throw new Error("ensureRangedIndex: Unknown comparator");
        }
        let rif = helper["b" /* RangedIndexFactoryMap */][indexTypeName];
        let comparator = helper["a" /* ComparatorMap */][comparatorName];
...
/* harmony import */ var _event_emitter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(3);
/* harmony import */ var _collection__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(2);
/* harmony import */ var _common_plugin__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(1);
/* harmony import */ var _helper__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(0);
    ...
    constructor(filename = "loki.db", options = {}) {
        ...
        if (options.comparatorMap) {
            for (let c in options.comparatorMap) {
                _helper__WEBPACK_IMPORTED_MODULE_3__[/* ComparatorMap */ "a"][c] = options.comparatorMap[c];
            }
        }
        ...
        if (options.rangedIndexFactoryMap) {
            for (let rif in options.rangedIndexFactoryMap) {
                _helper__WEBPACK_IMPORTED_MODULE_3__[/* RangedIndexFactoryMap */ "b"][rif] = options.rangedIndexFactoryMap[rif];
            }
        }
...
// EXTERNAL MODULE: ./packages/loki/src/loki.ts
var loki = __webpack_require__(4);

// EXTERNAL MODULE: ./packages/loki/src/collection.ts + 4 modules
var collection = __webpack_require__(2);

// CONCATENATED MODULE: ./packages/loki/src/index.ts


loki["a" /* Loki */]["Collection"] = collection["a" /* Collection */];

/* harmony default export */ var src = (loki["a" /* Loki */]);
 ...
 */
class ionic_file_storage_IonicFileStorage {
    constructor(file, options = {}) {
        // private static readonly TAG = "IonicFileStorage";
        this.mode = "reference"; // "reference" or "basic"
        this.file = file;
     ...
     */
    static register() {
        common_plugin["a" /* PLUGINS */]["IonicFileStorage"] = ionic_file_storage_IonicFileStorage;
    }
    /**
     * Deregisters the fs storage as plugin.
     */
    static deregister() {
        delete common_plugin["a" /* PLUGINS */]["IonicFileStorage"];
    }
    /**
    ...
    loadDatabase(dbName) {
        ...
        return this.file.checkFile(this.dbLocation, _name).then(() => {
            return this.file.readAsText(this.dbLocation, _name).then((text) => {
                const _loki = new loki["a" /* Loki */]();
                _loki.loadJSON(text);
                return _loki;
            });
        }).catch(() => {
            ionic_file_storage_IonicFileStorage.error("loadDatabase", "does not exist, creating");
            return this.file.createFile(this.dbLocation, _name, true).then(() => {
                return new loki["a" /* Loki */]();
            });
        });
    }
    /**
    ...
    saveDatabase(dbName, dbString) {
        ionic_file_storage_IonicFileStorage.log("saveDatabase");
        const _name = (this.dbName || dbName);
        ionic_file_storage_IonicFileStorage.log("saveDatabase:", _name, dbString.substring(0, 100));
        return this.file.writeFile(this.dbLocation, _name, dbString, this.dbWriteOptions);
    }
    /**
     * Save data to file, will throw an error if the file can't be saved
     * might want to expand this to avoid dataloss on partial save
    ...
    exportDatabase(dbName, dbRef) {
        ionic_file_storage_IonicFileStorage.log("exportDatabase");
        const _name = (this.dbName || dbName);
        ionic_file_storage_IonicFileStorage.log("exportDatabase", _name, dbRef.serialize({ serializationMethod: "normal" }).substring(0, 100));
        return this.file.writeFile(this.dbLocation, _name, dbRef.serialize({ serializationMethod: "normal" }), this.dbWriteOptions);
    }
    /**
...
//# sourceMappingURL=lokidb.ionic-file-storage.js.map